...
namespace Com.Github.Zycrophat.FileWatcherService.Filewatcher.Filesystemwatcher
{
    ...
    public class BufferingFileSystemWatcher : Component
    {
        ...
        private void NotifyExistingFiles()
        {
            ...
            var fileInfos = new DirectoryInfo(Path).EnumerateFiles(Filter, searchSubDirectoriesOption);
            fileInfos = OrderByOldestFirst ? fileInfos.OrderBy(fi => fi.LastWriteTime) : fileInfos;

            foreach (var fileInfo in fileInfos)
            {
                InvokeHandler(_onExistedHandler, new FileSystemEventArgs(WatcherChangeTypes.All, fileInfo.DirectoryName, fileInfo.Name));
                InvokeHandler(_onAllChangesHandler, new FileSystemEventArgs(WatcherChangeTypes.All, fileInfo.DirectoryName, fileInfo.Name));
            }
